...
successful, and the C<$wake_up> value is non-zero, then C<find_prompt> will
"hit the return key" to try to get some output. This process is retried
according to the value of C<$wake_up> (i.e. 1, 2, 3, etc), and of not
successful will die.
